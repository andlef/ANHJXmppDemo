...
//add by paul.an.  12162013

- (BOOL)isChatAttachmentAudio {
    return [[[self attributeForName:@"attaType"] stringValue] isEqualToString:@"audio"];
}
- (BOOL)isChatAttachmentImage {
    return [[[self attributeForName:@"attaType"] stringValue] isEqualToString:@"image"];
}
- (BOOL)isMessageWithAttachment {

    NSString *body = [[self elementForName:@"attachment"] stringValue];
    
    return ( [body length] > 0 );
}

- (BOOL)isMessageWithAttaType {
    
    NSString *body = [[self elementForName:@"attaType"] stringValue];
    
    return ( [body length] > 0 );
}
- (BOOL)isMessageWithAudio {
    
    NSString *body = [[self elementForName:@"attaAudio"] stringValue];
    
    return ( [body length] > 0 );
}
- (BOOL)isMessageWithImage {
    
    NSString *body = [[self elementForName:@"attaImage"] stringValue];
